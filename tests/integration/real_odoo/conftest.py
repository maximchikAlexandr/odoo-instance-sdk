...
from scripts.real_odoo_ci import capture_cleanup
...
def _finalize(runtime: E2ERuntime, primary_failure: BaseException | None = None) -> None:
    if runtime.finalized:
        return
    with capture_cleanup(runtime):
        _cleanup(runtime, primary_failure)


def _cleanup(runtime: E2ERuntime, primary_failure: BaseException | None = None) -> None:
    ...
    try:
        ...
        errors.append(error)
    # Successful cleanup is terminal even when the expected primary error is re-raised.
    runtime.finalized = not errors or (len(errors) == 1 and errors[0] is primary_failure)
    if not errors:
        return
    if len(errors) == 1:
        raise errors[0]
