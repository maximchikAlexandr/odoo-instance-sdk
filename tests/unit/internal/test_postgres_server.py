...
from odoo_instance_sdk.execution import ExecutionPlan, canonical_plan_bytes
...
def _captured_context(
    ...
    *,
    ...
    steps = build_server_summary_steps(_Cluster(), timeout=timeout)
...
def test_summary_tries_each_candidate_once_and_shares_remaining_budget(
    ...
    summary = collect_server_summary(
        _Cluster(), timeout=1.0, monotonic=clock, context=context, steps=steps
    )
...
def test_summary_privilege_failure_precedes_missing_database_regardless_of_order(
    ...
    summary = collect_server_summary(
        _Cluster(), monotonic=lambda: 0.0, context=context, steps=steps
    )
...
@pytest.mark.unit
def test_server_summary_steps_are_captured_with_absolute_argv_and_same_candidates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("odoo_instance_sdk.internal.pg.builder.shutil.which", lambda _: "/psql")
    steps = build_server_summary_steps(_Cluster(), timeout=3.0)
...
def test_server_summary_database_is_private_but_public_projection_is_stable(
    ...
    monkeypatch.setattr("odoo_instance_sdk.internal.pg.builder.shutil.which", lambda _: "/psql")
    first = build_server_summary_steps(_Cluster())
    ...
    second = build_server_summary_steps(_Cluster())
