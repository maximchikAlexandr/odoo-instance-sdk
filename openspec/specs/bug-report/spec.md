...
## Purpose
Provide a guarded, reviewable workflow for preparing and publishing reproducible OdCLI issues without bypassing the public CLI/SDK boundary.
...
- **THEN** the result shows the target repo, title, exact body, labels `["bug"]`, SHA-256, `report_valid`, `submit_ready`, `report_errors`, and `submit_blockers`, and no file is written, no `gh` is invoked, and no reviewer is contacted
