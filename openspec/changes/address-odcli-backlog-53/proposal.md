...
- Add a guarded `odcli db drop <database>` workflow with cluster scoping, confirmation, dry-run, active-connection handling, default-database protection, and catalogue reconciliation.
...
- Persist and monitor runtimes owned directly by a project, exposing them through the existing typed snapshot, HTTP API, and dashboard alongside environment-owned runtimes.
