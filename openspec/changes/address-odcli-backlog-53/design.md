...
A small internal loader will inspect exactly `<resolved-root>/.odcli/.env`, verify owner-only POSIX permissions where meaningful, and implement the exact grammar in the `project-init` delta: ASCII identifier keys, blank/comment lines, unquoted values without inline-comment semantics, literal single quotes, and double quotes with only `\\`, `\"`, `\n`, `\r`, and `\t` escapes. It will reject duplicates, multiline/trailing syntax, interpolation, substitutions, unknown escapes, NULs, invalid UTF-8, and unreadable files. Values are merged underneath an immutable snapshot of `os.environ`, never written to global `os.environ`.

The existing `captured_child_environment()`/`sanitized_child_environment()` boundary remains authoritative. Ordinary file values may augment Odoo runtime children only; Git, PostgreSQL, Docker/Compose, editor/browser, package/build, and other children retain their purpose-built environments. Process values override file values. `ODCLI_TEST_MASTER_PASSWORD` is secret input consumed by restore coordination and stripped before every spawn and public projection. The existing secret classifier redacts credential-like keys/values but never expands their propagation rights. Init/doctor documentation and tests will ensure `.odcli/.env` is ignored and owner-readable only.
...
A CLI-private/internal command builder, outside the public resource surface, will use the existing PostgreSQL transport. It preserves `DatabaseResource.drop/drop_command` as call-time-master-password Odoo HTTP manager operations and keeps the discovered public-method inventory unchanged. Planning resolves the project cluster and exact quoted identifier, rejects the exact denylist `postgres`, `template0`, and `template1`, reads `pg_database.datistemplate`, configured-default status, and active sessions. Execution revalidates every safety observation immediately before mutation, optionally terminates only sessions attached to the exact non-template target, issues `DROP DATABASE` while connected to maintenance database `postgres`, and verifies absence. It then calls existing `record_database_dropped` exactly once; whether a new row is inserted remains governed by that helper's latest-event idempotency rule.

Confirmation and force policy stays in the CLI: Rich may prompt, while JSON/TOON are always noninteractive and reject normal execution without `--yes` using one `confirmation_required` envelope before SDK work; dry-run never requires consent. `--force-default` permits the protected configured database, and `--force-connections` permits session termination. Credentials use the existing child environment/private plan projection, never argv.
...
The catalogue receives a minimal durable `projects` registration keyed by the canonical repository/git identity already used for monitor project IDs, plus a generalized runtime table with `owner_kind` and `owner_id` uniqueness. Migration backfills project registrations from existing environments and copies `environment_runtime` rows as environment-owned records before retiring the old table. A successful non-preview `init` writes registration. Ordinary resolution, read-only commands, monitor collection, and every dry-run remain side-effect-free. Normal foreground execution/lifecycle may upsert registration immediately before its permitted runtime write; this is also the legacy project-only registration path, so a previously initialized checkout with no environments becomes discoverable after a normal foreground run but not after preview. No registration write creates an environment or lifecycle event.

`from_environment()` and `from_project()` attach the same private runtime-binding type with different owners. Foreground lifecycle upserts/clears through one catalogue API. Monitor planning begins from registered projects, joins optional environments and both runtime owner kinds, and reuses current process, readiness, Git, storage, and PostgreSQL collectors. Snapshot schema version 4 adds exactly `ProjectSummary.runtime: RuntimeMetrics | None`; null means no persisted live/stale project runtime record, while a present stopped value uses the canonical stopped/null `RuntimeMetrics` semantics. Environment snapshot fields stay compatible. OpenAPI is regenerated from msgspec, then the generated TypeScript client and React project card render the field.
...
5. Add the guarded CLI-private PostgreSQL database-drop operation while preserving public SDK drop behavior, and verify mutations only on disposable PostgreSQL clusters.
