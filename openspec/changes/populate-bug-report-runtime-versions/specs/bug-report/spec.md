...
- **THEN** the catalog row is not used, no probes from its project runtime are planned, and draft initialization retains the safe `unknown` fallback unless an independently valid bounded manifest is found
