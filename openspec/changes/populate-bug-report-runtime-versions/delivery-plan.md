...
- Уверенность: средняя. OpenSpec, production path, process boundary, project/runtime resolution и тестовые аналоги изучены; основные неопределённости связаны с аккуратным встраиванием optional PostgreSQL steps, их ledger accounting и эквивалентностью filesystem-only Git identity на редких marker layouts.
...
- Переиспользование существующих контрактов `internal.context`, `internal.repo_key`, `internal.project_runtime`, `internal.pg.server`, `resources.postgres` и `internal.proc` без второго Git parser/runner/provider hierarchy.
- Критические shared files: `src/odoo_instance_sdk/bug_report.py`, `src/odoo_instance_sdk/internal/context.py`, `src/odoo_instance_sdk/internal/repo_key.py`, `tests/unit/test_bug_report.py`, `tests/unit/internal/test_context.py`, `tests/unit/internal/test_repo_key.py`, а при доказанной необходимости — непосредственно связанные process/output contract tests.
...
- Construction snapshot использует только filesystem/read-only catalog и один строгий marker parser рядом с compatibility-preserving `internal.repo_key.git_common_dir()`: первый `.git` entry задаёт manifest boundary даже при invalid marker, catalog row требует существующий валидный marker/target и точное совпадение root/worktree/common-dir; command-backed `resolve_project()` и `git_worktree.rev_parse_*` на этом пути запрещены.
...
- Command-plan tests доказывают immutable capture, redaction, read-only/non-shell execution, time bounds и полное consume/skip accounting; отдельный spawn trap падает при любом `execute()`/`spawn()` во время command construction. Snapshot regressions отвергают catalog row с missing/stale marker и не позволяют nested unrelated repository пересечь собственный root ради parent manifest/runtime.
