...
- [ ] 1.3 [T03] Add the `e2e_smoke` and `e2e_full` pytest markers, immutable pin manifest, phase-budget constants, and supported-platform validation.
- [ ] 1.4 [T04] Add unit checks for pin parsing, matrix generation, budget classification, and fail-closed prerequisite reporting.
...
- [ ] 3.2 [T11] Checkout pinned Odoo source through public `env checkout --create-venv`, assert commit/config/worktree/venv identity, run `env sync`, and verify dependencies.
...
- [ ] 5.2 [T21] Add the scheduled/manual Linux-amd64 full job with pinned Odoo/Python/uv inputs, 25-minute timeout, content-addressed source/uv caches, and no mutable-state caches.
...
- [ ] 6.3 [T26] Verify every `PUBLIC_LEAF_CASES` row maps to exactly one disposition/evidence entry, all spec scenarios have executable evidence, and no production runner, public API, Enterprise input, mutable backup, or hidden prerequisite was added.
