...
Schema MUST versionироваться через `PRAGMA user_version`. Первая schema version MUST быть `1`.

Каждая public catalog operation MUST использовать короткую transaction, `foreign_keys=ON`, WAL mode и busy timeout 5000 ms.
