...
Lock files и `flock` MUST быть internal implementation `EnvironmentResource`/instance runtime, не public module и не CLI `LockManager`. CLI не вызывает lock API напрямую.
...
- run/shell автоматически sync'ят только при changed inputs.
...
Reconciliation (quick check) ALWAYS выполняется в `env list` (не требует `--verify`). `EnvironmentResource.list(verify=True)` — расширенная сверка (дополнительные filesystem checks); `verify=False` (default) — quick reconciliation как описано.
...
`--older-than` фильтрует по `last_used_at` и не удаляет автоматически. Duration format: `<number><unit>` где unit = `d` (days), `h` (hours), `m` (minutes). Examples: `14d`, `2h`, `30m`. Invalid format → error.
