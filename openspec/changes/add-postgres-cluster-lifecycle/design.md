...
- **Image trust**: `approve_image(repo@sha256:...)` resolves `docker image pull` plus `docker image inspect` and accepts only the exact OCI RepoDigest. The per-project `approved-images.json` allowlist is user-data, mode `0600`, never in the repository. Every compose start resolves the mutable manifest reference again, compares it to approval, and generates compose from the immutable digest while holding the project lifecycle lock.
