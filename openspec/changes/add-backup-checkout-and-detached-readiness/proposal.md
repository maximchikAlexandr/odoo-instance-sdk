...
- Treat origin approval variables as legacy and ignore them; remove separate origin-pin checks for named and legacy sources.
...
- Add optional readiness to environment-bound detached launch.
- Add backup pinning, age-based pruning and opt-in post-success cleanup through the existing catalog.
...
Extend existing configuration, preparation/checkout commands, backup facade, catalog migrations and canonical CLI inventory. Preserve legacy single-source behavior and disabled automatic retention. No new runtime dependency is expected.
