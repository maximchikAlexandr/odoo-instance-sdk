...
The existing `run_or_preview(lambda: command, emit_normal=False)` stays unchanged. The callback first calls the shared `ready_instance()`, which resolves the SDK/environment, calls `client.instance.from_environment(environment)`, and returns the instance under the current contract. Port preflight then runs; a conflict returns with that resolved instance unused and without foreground command construction, `record_use`, or process launch. On a free port the callback captures `run_foreground_command(args=...)` exactly once. Dry-run then previews that command without `record_use`; normal execution calls `record_use` once after command capture and before `run_or_preview` executes it. Normal execution inherits native streams and exits with the command's integer result; dry-run emits the captured plan through the existing Rich/JSON/TOON boundary. This slice does not split, defer, or otherwise refactor instance creation out of `ready_instance()`.
...
Alternative considered: defer `from_environment()` until after the port preflight. Rejected because it changes the shared `ready_instance()` contract outside GitHub #35 and is unnecessary to preserve the required no-command/no-launch conflict behavior.
