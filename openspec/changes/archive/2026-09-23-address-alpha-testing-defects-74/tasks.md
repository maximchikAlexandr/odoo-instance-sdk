...
- [x] 17.1 Write `openspec/changes/address-alpha-testing-defects-74/ef-inventory.md` using design D17 heuristics, with merge-base coverage percents.
- [x] 17.2 For each, decide rewritten/replaced/deleted with a reason; preserve behavioural coverage elsewhere; reject "did not crash" as a replacement assertion.
- [x] 17.3 Enforce line and branch coverage not below baseline; preserve requirement and regression coverage.
- [x] 17.4 No new skips, xfails, or weakened assertions without justification; no cosmetic gaming.
- [x] 17.5 Run the full `make pr` gate and confirm all test/CI gates pass.
...
- [x] 19.1 Update README/CLI documentation for `env rm --force-connections`, `init` new options, `db reset-admin-password` secret boundary, optional `test_instance.database`, long-restore progress, `module update` diagnostics, `--version` VCS revision, `bug-report`, `update`, transport centralization, and `run --detach` logfile.
